/**
 * This is the game method for Logic1 with RGS_SPS_EES agent.
 * @author 170024030
 *
 */
public class Logic2 {
	...
	public static void main(String[] args) {
		...
		try {
			...
			Agent_RGS_SPS_EES agent = new Agent_RGS_SPS_EES(map);
			...
			while(!agent.unknown.isEmpty()) {
				/* if SPS doesn's work, resort to EES
				 * if EES doesn's work, resort to RGS
				 */
				if (!agent.sps() && !agent.ees()) {
					agent.rgs();
				}
				if (!agent.isSafe) {
					break;
			...
			if (agent.isSafe) {
				...
				System.out.println("Number of EES used: " + agent.eesCount);
			}
			else {
